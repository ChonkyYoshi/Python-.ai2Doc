...
def FixAiFileList(AiFileList: list):
    for index, AiFile in enumerate(AiFileList):
        ...
        if not AiFile.is_file():
            gui.popup_error(
                f'"{AiFile.name}" {config["errors"]["Not_file"]}',
        ...
        if AiFile.suffix != ".ai":
            gui.popup_error(
                f'"{AiFile.name}" {config["errors"]["Not_ai"]}', title="File is not .ai"
            )
            AiFileList.pop(index)
            continue
    return AiFileList
...
def FixWordFileList(WordFileList: list):
    for index, WordFile in enumerate(WordFileList):
        ...
        if not WordFile.is_file():
            gui.popup_error(
                f'"{WordFile.name}" {config["errors"]["Not_file"]}',
                title="File doesn't exist",
            )
            WordFileList.pop(index)
            continue
        if WordFile.suffix != ".docx":
            gui.popup_error(
                f'"{WordFile.name}" {config["errors"]["Not_docx"]}',
...
def ExtractText(AiApp, WordApp, AiFile: Path, Hidden: bool, Locked: bool, PDF: bool):
    ...
    WordFile.SaveAs2(
        f"{AiFile.parent.as_posix()}/Strings_{finalname}.docx", FileFormat=12
    )
    WordFile.Close()


def ImportText(
    AiApp, AiFile: Path, WordApp, WordFile: Path, Hidden: bool, Locked: bool, PDF: bool
):
...
def Pseudo(AiApp, AiFile: Path, Hidden: bool, Locked: bool, PDF: bool):
    ...
    for index, frame in enumerate(AiDoc.TextFrames):
        ...
        yield (
            f"PseudoTranslating text, Segment {index + 1} of"
...
def Collapsible(
    layout, key, title="", arrows=(gui.SYMBOL_DOWN, gui.SYMBOL_UP), collapsed=False
):
    ...
    return gui.Column(
        [
            [
                ...
                gui.T(title, enable_events=True, key=key + "-TITLE-"),
            ],
            [
                gui.pin(
                    gui.Column(layout, key=key, visible=not collapsed, metadata=arrows)
                )
            ],
        ],
        pad=(0, 0),
...
layout = [
    ...
    [
        gui.Text(
            text="Select 'Extract' or 'Import' to start.", key="-Info-", size=(65, 15)
        )
    ],
    [
        gui.InputText(
            default_text="Path to .ai files.",
            key="-AiPath-",
            visible=False,
        ),
        gui.FilesBrowse(
    ...
    [
        gui.InputText(
            default_text="Path to .docx files.", key="-DocPath-", visible=False
        ),
        gui.FilesBrowse(
            button_text="Browse",
            target="-DocPath-",
    ...
    [Collapsible(layout=Options, key="Options", title="Options", collapsed=True)],
...
while True:
    ...
    match event:
        ...
        case "Options-BUTTON-":
            ...
            window["Options" + "-BUTTON-"].update(
                window["Options"].metadata[0]
                if window["Options"].visible
                else window["Options"].metadata[1]
            )
        ...
        case "-Start-":
            ...
            match Task:  # type: ignore
                case "Extract":
                    ...
                    if len(AiFileList) == 0:
                        gui.popup_error(
                            config["errors"]["No_ai_left"], title="No .ai file"
                        )
                        continue
                    ...
                    for AiFileindex, AiFile in enumerate(AiFileList):
                        ...
                        for step in ExtractText(
                            ...
                            window["-PBar-"].update(
                                current_count=(AiFileindex + 1) / len(AiFileList)
                            )
                    WordApp.Quit()
                    AiApp.Quit()
                case "Import":
                    ...
                    if len(WordFileList) == 0:
                        gui.popup_error(
                            config["errors"]["No_docx_left"], title="No .docx file"
                        )
                        continue
                    if len(AiFileList) != len(WordFileList):
                        gui.popup_error(
                            ...
                            title="Different amount of files",
                        )
                    window["-PStep-"].update(value="Opening Illustrator and Word")
                    ...
                    if len(NoMatch) != 0:
                        gui.popup(
                            f"The following files could not be matched and were skipped:\n{NoMatch}"
                        )  # noqa: E501
                case "Pseudo":
                    AiFileList = FixAiFileList(AiFileList)
                    if len(AiFileList) == 0:
                        ...
                        continue
                    window["-PStep-"].update(value="Opening Illustrator ")
